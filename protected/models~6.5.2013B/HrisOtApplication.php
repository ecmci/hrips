...
class HrisOtApplication extends BaseHrisOtApplication
{
    	...
    	public function approveOT($action='0',$denial_reason='',$to_hr='0'){
    		...
    		switch(Yii::app()->user->getState('access_lvl_id')){
    			...
    			case HrisAccessLvl::$SUPERVISOR : 			
					...
					$this->next_lvl_id = ($action == '1') ? HrisAccessLvl::$MANAGER : HrisAccessLvl::$ULTIMATELY_DENIED;
...
//     		
  ...
  public function renderApprovedHours($data, $row){
	...
	echo CHtml::dropDownList("ot[$row][hours]",$ex[0],$hours,array('style'=>'width:50px;')).' : '.CHtml::dropDownList("ot[$row][minutes]",$d,$minutes,array('style'=>'width:50px;'));
  }
