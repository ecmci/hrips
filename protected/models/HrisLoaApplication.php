...
class HrisLoaApplication extends BaseHrisLoaApplication {
    ...
    public function getLOAForApproval(){
        $curr_emp_id = Yii::app()->user->getState('emp_id');
        $curr_usr_acl = Yii::app()->user->getState('access_lvl_id');
        $curr_dept_id = Yii::app()->user->getState('dept_id');
        $criteria = new CDbCriteria;

        //get those by user access level
        switch($curr_usr_acl){
            case HrisAccessLvl::$EMPLOYEE: //as reliever
                $criteria->condition="reliever_id='$curr_emp_id' AND reliever_approve IS NULL";         
                break;
            case HrisAccessLvl::$SUPERVISOR:
            case HrisAccessLvl::$MANAGER:
                $criteria->condition="(next_lvl_id=".HrisAccessLvl::$SUPERVISOR." AND dept_id=$curr_dept_id) OR (reliever_id='$curr_emp_id' AND reliever_approve IS NULL)";
                break;
            case HrisAccessLvl::$HR:
                $criteria->condition="(next_lvl_id=".HrisAccessLvl::$HR.") OR (reliever_id='$curr_emp_id' AND reliever_approve IS NULL)";
                break;
        }
        
        //for everything else, get only those which are for reliever, supervisor, manager and hr approvals
        $criteria->addInCondition('next_lvl_id',array(HrisAccessLvl::$RELIEVER,HrisAccessLvl::$SUPERVISOR,HrisAccessLvl::$MANAGER,HrisAccessLvl::$HR));
    ...
    public function getLOAForApprovalORIG() {
        $criteria = new CDbCriteria;
    ...
    private function logHoursORIG() {
        try {
            ...
            echo "normalized_start=" . $normalized_start['TimeIn'] . ' | ' . "normalized_end=" . $normalized_end['TimeOut'];
            exit();
    ...
    private function logHours000000000000000000000() {
        try {
            ...
            //$end_datetime = $this->to_datetime;
            $emp_id = $this->emp_id;
            Yii::log("start_datetime = $start_datetime | end_datetime = $end_datetime", 'error', 'app');
    ...
    public static function testlogHours() {
        try {
            ...
            if ($shifts == NULL or sizeof($shifts) == 0)
                throw new Exception('No schedule set.');
