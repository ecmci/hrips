...
class PdsPrintController extends Controller
{
	...
	ini_set('max_execution_time', 120); // 120 seconds (2 minutes)
		...
		$childCriteria=new CDbCriteria;$childCriteria->addCondition('EmpID ='.$id);
		$myChildren=EmpChildren::model()->findAll($childCriteria);
		$educCriteria=new CDbCriteria;$educCriteria->addCondition('EmpID ='.$id.'','AND');$educCriteria->addCondition('NameofSchool !=""');
		$EducBg=EmpEducbg::model()->findAll($educCriteria);
		$civilCriteria=new CDbCriteria;$civilCriteria->addCondition('EmpID ='.$id.'','AND');$civilCriteria->addCondition('CareerService !=""');
		$CivilService=EmpCivilservice::model()->findAll($civilCriteria);
		$workCriteria=new CDbCriteria;$workCriteria->addCondition('EmpID ='.$id.'','AND');$workCriteria->addCondition('PositionTitle !=""');
		$WorkExps=EmpWorkexp::model()->findAll($workCriteria);
		$volCriteria=new CDbCriteria;$volCriteria->addCondition('EmpID ='.$id.'','AND');$volCriteria->addCondition('NameAddressOrg !=""');
		$VolWorks=EmpOrganization::model()->findAll($volCriteria);
		$trainingCriteria=new CDbCriteria;$trainingCriteria->addCondition('EmpID ='.$id.'','AND');$trainingCriteria->addCondition('SeminarTitle !=""');
		...
		$refCriteria=new CDbCriteria;$refCriteria->addCondition('EmpID ='.$id.'','AND');
