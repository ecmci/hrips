...
class WebApp
{
  ...
  /**
   ...
   public static function processEmailQueue(){
      ...
      if(!empty($emails)) self::sendEmail($emails);       
   }
      ...
      try{
         ...
         $records = $tcsvr->createCommand()
                    ...
                    //->andWhere("TimeIn >= '$start'")
					         //->andWhere("TimeIn >= '2013-06-12'")
                   //->andWhere("TimeOut <= '2013-06-09 23:55:00'")
					         //->andWhere("EmployeeId = '1110'")
                    ->order("EmployeeId asc, TimeIn asc")
                    ->queryAll();
          $i = 0;
		  Yii::log("Extracting OT Records as of $start ...",'info','app');
          foreach($records as $record){
              //$employee = HrisUsers::model()->find(array('condition'=>"emp_id = '".$record['EmployeeId']."'"));
		  ...
		  //Yii::log('Importing OT from Timeclock since '.$start,'info','app');
  ...
  /**
	...
	public static function getScheduledShiftsWithin($start_datetime,$end_datetime,$emp_id){
		...
		//Yii::log('start='.$start_datetime,'info','app');
		//Yii::log('end='.$end_datetime,'info','app');
		try{
			$tcsvr = Yii::app()->tcdb;
			$res = $tcsvr->createCommand()
					->select("TimeIn,TimeOut")
	...
	public function getInOutDatetimes($aDatetime,$emp_id){
		...
		//$aDatetime = date('Y-m-d',strtotime($aDatetime));
		...
		//echo "<pre>";print_r(	$normalized_scheduled_shifts);echo "</pre>";   exit();

		$user_ts = strtotime($aDatetime);
		$at = -1;
		foreach($normalized_scheduled_shifts as $i=>$shift){
      // echo "at:$i".'<br>';
	  // echo "I: ".$shift['TimeIn']."<br>";
      // echo "?: ".$aDatetime."<br>";
      // echo "O: ".$shift['TimeOut'].'<br><br>';
  ...
  public static function getTimeDifferenceInHrs($date1,$date2){
	...
	public static function getMenuItems()
	{
		...
		return
			array(
				...
				array('label' => 'How to Setup', 'icon' => '-101px -487px', 'url' => array('/site/page', 'view' => 'system'),
					'items' => array(
						array('label' => 'Print', 'url' => '#1'),
						array('label' => 'Export as...', 'url' => '#',
							'items' => array(
								array('label' => 'Excel', 'url' => '#excel'),
								array('label' => 'PDF', 'url' => '#pdf'),
							),
						),
					),
				),
				array('label' => 'Operations', 'icon' => '-72px -359px', 'url' => array('/site/page', 'view' => 'operations'),
					'items' => array(
						array('label' => 'Start New Transaction', 'url' => '#4'),
						array('label' => 'List Transactions', 'url' => '#5'),
						array('label' => 'Employees', 'url' => '#6'),
					),
				),
    ...
    /*
		return		
		array(
				array('label'=>'For Approval <span class="caret"></span>', 
					'url'=>array('#'),
					'itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"),
					'items'=>array(						
						array('label'=>'Leave of Absence','url'=>array('/hrisLoaApplication/formyapproval')),
            array('label'=>'Overtime','url'=>array('/hrisOtApplication/formyapproval'),'visible'=>Yii::app()->user->getState('access_lvl_id')!=HrisAccessLvl::$EMPLOYEE),						
					),
					'visible'=>!Yii::app()->user->isGuest,
				),	
        array('label'=>'My Application <span class="caret"></span>', 
					'url'=>array('#'),
					'itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"),
					'items'=>array(						
						array('label'=>'Leave of Absence','url'=>array('/hrisLoaApplication')),
            array('label'=>'Overtime','url'=>array('/hrisOtApplication')),						
					),
					'visible'=>!Yii::app()->user->isGuest,
				),	
            ...
            array('label'=>'Email Templates','url'=>array('/admin/hrisEmailTemplate')),						
					),
					'visible'=>Yii::app()->user->getState('access_lvl_id')==='4',
				),				
				array('label'=>'Login', 'url'=>array('/site/login'), 'visible'=>Yii::app()->user->isGuest),				
				array(
					'label'=>'Hello, '.Yii::app()->user->name. '<span class="caret"></span>', 
					'url'=>array('myaccount/user/view/id/'.Yii::app()->user->id),
					'itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"),
					'items'=>array(
            array('label'=>'Change Password','url'=>array('#')),						
      ...
      */
      ...
      return array(
		...
		array('label'=>'Reports<span class="caret"></span>','url'=>array('#'),'visible'=>(!Yii::app()->user->isGuest and Yii::app()->user->getState('access_lvl_id')!=HrisAccessLvl::$EMPLOYEE),
            'itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"),
			'items'=>array(						
		...
		// array(
					// 'label'=>'Howdy, '.Yii::app()->user->name. '<span class="caret"></span>', 
					// 'url'=>array('myaccount/user/view/id/'.Yii::app()->user->id),
					// 'itemOptions'=>array('class'=>'dropdown','tabindex'=>"-1"),'linkOptions'=>array('class'=>'dropdown-toggle','data-toggle'=>"dropdown"),
					// 'items'=>array(
						// array('label'=>'Change Password','url'=>array('admin/hrisUsers/resetpassword/id/'.Yii::app()->user->getState('emp_id'))),							
						//array('label'=>'Logout','url'=>array('/site/logout')),						
					// ),
					// 'visible'=>!Yii::app()->user->isGuest,
				// ),
