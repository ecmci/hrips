...
public function accessRules() {
	return array(
			...
			array('allow', 
				'actions'=>array('resetpassword','updateprofilepic'),
				'users'=>array('@'),
				),
			array('deny', 
				'users'=>array('*'),
    ...
    if(isset($_POST['HrisUsers'])){
       $model->attributes=$_POST['HrisUsers'];
       $model = HrisUsers::model()->findByPk($model->emp_id);               
       $uploadedFile=CUploadedFile::getInstance($model,'image');
       $salt = time();
       $fileName = "{$salt}-{$uploadedFile}";
       $model->image = $fileName;
       if(!empty($uploadedFile) and $model->save(false)) {
